  ...
  <nut-dialog title="新增" content="新增" v-model:visible="visibleDialog" @cancel="onDialogCancel" @ok="onDialogOk">
    <template v-slot:default>
      ...
      <nut-input
          ...
          clearable
          clearSize="14"/>
      <nut-cell title="请选择专业" :desc="pickerDesc" @click="pickerShow = true"></nut-cell>
    </template>
  </nut-dialog>
  <nut-popup position="bottom" v-model:visible="pickerShow">
    <nut-picker
        :columns="columns" title="选择专业"
...
const pickerConfirm = ({ selectedValue, selectedOptions }) => {
  basicData.professionalId = selectedValue[selectedValue.length - 1];
...
const onDialogOk = () => {
  commonPost("/classManager/addClassManager", basicData).then(res => {
    if (res.code == 200) {
      basicData.code = "";
      basicData.name = "";
      pickerDesc.value = "";
...
const onEditDialogOk = () => {
  commonPut("/classManager/updateClassManager", basicData).then(res => {
    if (res.code == 200) {
      basicData.id = "";
      basicData.code = "";
      basicData.name = "";
...
const onDelOk = () => {
  commonDelete("/classManager/deleteClassManager/" + basicData.id).then(res => {
    if (res.code == 200) {
      basicData.id = "";
      basicData.code = "";
      basicData.name = "";
...
const getData = async (data?: any) => {
  ...
  commonGet("/classManager/list", data).then(res => {
        if (res.code == 200) {
          res.data.forEach((item) => {
                item.operation = () => {
                  return h(
  ...
  columns.value.forEach(async item => {
    item.children = await getList("/professional/list", {departmentId: item.value});
  });
}
