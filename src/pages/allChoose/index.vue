    ...
    <nut-collapse v-model="activeName" :accordion="true">
      <nut-collapse-item v-for="item in data" :title="item.name + ' ' + item.code" :name="item.id"
        ...
        <div style="display: flex; justify-content: right">
          <nut-button v-if="isChoose(item.id)" @click="chooseClassDanger(item.id)" type="danger" size="mini">退选</nut-button>
          <nut-button v-else @click="chooseClass(item.id)" type="primary" size="mini">选课</nut-button>
        </div>
      </nut-collapse-item>
    </nut-collapse>
  </div>
  <nut-dialog title="选课" content="确定选课吗?" v-model:visible="visibleDialog" @cancel="visibleDialog = false" @ok="onOk" />
...
const searchClick = () => {
  getData({search: inputValue.value, departmentId: store.userInfo.departmentId})
}
...
const isChoose = (id) => {
  if (myChoose.value.length == 0) {
    return false;
  }
  let index = myChoose.value.findIndex(item => {
    return item.curriculumId == id
  })
  return index != -1;
}
...
const getMyChoose = () => {
  commonGet("/curriculum/user/list", {userId: handleBigNumber(store.userInfo.id)}).then(res => {
    if (res.code == 200) {
      res.data.forEach(item => {
        item.userId = handleBigNumber(item.userId);
        item.curriculumId = handleBigNumber(item.curriculumId);
      })
      myChoose.value = res.data;
    } else if (res.code == 505) {
      router.replace("/login");
    } else {
      openToast(res.msg);
...
onMounted(async () => {
  getData({departmentId: handleBigNumber(store.userInfo.departmentId)});
  getMyChoose();
})
...
const onOk = () => {
  commonPost("/curriculum/user/addCurriculumUser", {userId: store.userInfo.id, curriculumId: curriculumId.value}).then(res => {
    if (res.code == 200) {
      openToast("选课成功!");
      getData({departmentId: handleBigNumber(store.userInfo.departmentId)})
      getMyChoose();
