...
<!--    <nut-cell title="请选择年级" :desc="pickerGradeDesc" @click="pickerGradeShow = true"></nut-cell>-->
  ...
  <nut-popup position="bottom" v-model:visible="pickerDepartmentShow">
    <nut-picker
        :columns="departmentColumns" title="选择院系"
        @cancel="pickerDepartmentShow=false"
        @confirm="pickerDepartmentConfirm"
    >
    </nut-picker>
  </nut-popup>
  <nut-popup position="bottom" v-model:visible="pickerGradeShow">
    <nut-picker
        :columns="columns" title="选择年级"
        @cancel="pickerGradeShow=false"
        @confirm="pickerConfirm"
    >
    </nut-picker>
  </nut-popup>
  <nut-popup position="bottom" v-model:visible="pickerTeacherShow">
    <nut-picker
        :columns="teacherColumns" title="选择任课教师"
        @cancel="pickerTeacherShow=false"
        @confirm="pickerTeacherConfirm"
    >
    </nut-picker>
  </nut-popup>
...
const getData = (searchData?: any) => {
  if (parseInt(store.userInfo.role) == 1) {
    if (searchData) {
      searchData.departmentId = handleBigNumber(store.userInfo.departmentId);
    } else {
      searchData = {
        departmentId: handleBigNumber(store.userInfo.departmentId)
      }
    }
  }
  commonGet("/curriculum/list", searchData).then(res => {
    if (res.code == 200) {
      res.data.forEach(item => {
...
onMounted(async () => {
  getData();
  departmentColumns.value = await getList("/departmentManager/list") as any;
...
const pickerDepartmentShow = ref(false);
const departmentColumns = ref([])
const pickerDepartmentConfirm = async ({selectedValue, selectedOptions}) => {
  editData.departmentId = selectedValue[0];
  pickerDepartmentDesc.value = selectedOptions.map((val: any) => val.text).join(',');
  pickerDepartmentShow.value = false;
  teacherColumns.value = await getList("/teacher/list", {departmentId: editData.departmentId}) as any;
  pickerTeacherDesc.value = "";
  editData.teacherId = "";
}

const pickerTeacherDesc = ref("");
const pickerTeacherShow = ref(false);
const teacherColumns = ref([])
const pickerTeacherConfirm = ({selectedValue, selectedOptions}) => {
  editData.teacherId = selectedValue[0];
  pickerTeacherDesc.value = selectedOptions.map((val: any) => val.text).join(',');
  pickerTeacherShow.value = false;
}
...
const pickerConfirm = ({selectedValue, selectedOptions}) => {
  editData.grade = selectedValue[0];
  pickerGradeDesc.value = selectedOptions.map((val: any) => val.text).join(',');
  pickerGradeShow.value = false;
}
