<template>
  ...
  <nut-dialog title="新增账号" content="新增账号" v-model:visible="visibleDialog" @cancel="onDialogCancel"
    ...
    <template v-slot:default>
      ...
      <nut-input
          ...
          clearable
          clearSize="14"/>
      <nut-cell title="请选择班级" :sub-title="popupDesc" @click="show = true"></nut-cell>
    </template>
  </nut-dialog>
  <nut-dialog title="删除账号" content="确定删除账号吗?" v-model:visible="visibleDelDialog" @ok="delOk"
              @cancel="delDialogCancel"/>
  <nut-dialog title="编辑账号" content="确定编辑账号吗?" v-model:visible="visibleEditDialog" @ok="editOk"
  ...
  <nut-fixed-nav
      @selected="fixedSelected" :overlay="false"
...
const delOk = () => {
  commonDelete("/student/deleteStudent/" + deleteUserId.value).then(res => {
    if (res.code == 200) {
      openToast(res.msg);
...
const editOk = () => {
  commonPut("/student/updateStudent", editData).then(res => {
    if (res.code == 200) {
      openToast(res.msg);
      getListData();
    } else if (res.code == 505) {
...
const onDialogOk = () => {
  commonPost("/student/addStudent", basicData).then(res => {
    if (res.code == 200) {
      getListData();
...
const popupConfirm = ({selectedValue, selectedOptions}) => {
  ...
  show.value = false
  basicData.classId = selectedValue[selectedValue.length - 1];
  editData.classId = selectedValue[selectedValue.length - 1];
}
...
onMounted(async () => {
  ...
  for (let i = 0; i < columns.value.length; i++) {
    let item = columns.value[i] as any;
    item.children = await getList("/professional/list", {departmentId: item.value}) as any;
    for (let j = 0; j < item.children.length; j++) {
      let subItem = item.children[j] as any;
      subItem.children = await getList("/classManager/list", {professionalId: subItem.value}) as any;
    }
  }
  getListData();
})
...
const getListData = (data?: any) => {
  if (parseInt(store.userInfo.role) == 1) {
    if (data) {
      data.departmentId = handleBigNumber(store.userInfo.departmentId);
    } else {
      data = {
  ...
  commonGet("/student/list", data).then(res => {
    if (res.code == 200) {
